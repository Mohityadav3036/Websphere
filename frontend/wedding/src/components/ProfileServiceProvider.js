...
function ProfileServiceProvider() {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/serviceprovider/profile`, {
        ...
        if (response.status === 200) {
          setUser({
            ...
            phone: response.data.serviceProvider.phone,
            newPassword: "",
          });
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/serviceprovider/updateprofile`, updateData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setTimeout(() => navigate("/serviceprovider/login"), 3000);
    } catch (err) {
